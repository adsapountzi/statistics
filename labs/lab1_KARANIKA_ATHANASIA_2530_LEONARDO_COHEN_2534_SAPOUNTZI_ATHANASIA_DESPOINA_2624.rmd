...
and RStudio is a convenient interf?ce.
...
begin wi?h the fundamental building blocks of R and RStudio: the interface, 
...
the commands that you've previ?usly entered.  Any plots that you generate will 
...
co?sole telling you the version of R that you're running.  Below that information
...
and interpreting t?e output. These commands and their syntax have evolved over
...
To get you started, enter the followin? command at the R prompt (i.e. right after
...
This command ?nstructs R to access the OpenIntro website and fetch some data: 
...
data set called `arbuthnot` that has ?2 observations on 3 variables. As you 
...
computation or some analysis you have performed. Note t?at because you are 
...
The Arbu?hnot data set refers to Dr. John Arbuthnot, an 18<sup>th</sup> century 
...
London for every year fr?m 1629 to 1710.  We can take a look at the data by 
...
different year: the first entry in each row is sim?ly the row number (an index 
...
that year, respectively. Use the scrollbar on the right side of the ?onsole 
...
think of them as the index that you see on t?e left side of a spreadsheet. In 
...
the object in your workspace. You can see the names of these ?olumns (or 
variables) by typing:
...
lot like fun?tions from math class; that is, invoking R commands means supplying
...
One advantage of RStudio is that it comes w?th a built-in data viewer. Click on
...
data set in the *Data Viewer* (upper left window). You can ?lose the data viewer
...
`5218` follows `[1]`, indi?ating that `5218` is the first entry in the vector. 
...
R has some powerful functions for making graphics. We can create ? simple plot 
...
circle. The plot ?tself should appear under the *Plots* tab of the lower right 
...
plot function specifies the variable ?or the x-axis and the second for the y-axis.
...
toggle between plots ?nd help files using the tabs at the top of that panel.

2.  Is there an apparent trend in the number of girls baptized over the years?  
    How would you describe it?
    
    the number of girls increases generally
...
to see the total number of baptisms in 1629. We ?ould repeat this once for each 
...
What you will see ?re 82 numbers (in that packed display, because we aren't 
...
plot of the total number of baptisms ?er year with the command
...
The proportion of newborns that are b?ys
...
Note that with R as with you? calculator, you need to be conscious of the order 
...
division, then the addition, giving you some?hing that is not a proportion.
...
commands, your so-called command history. You can also access it b? clicking on 
the history tab in the upper right panel.  This will save you a lot of typing in
the future.
```{r plot-propotion of boys, eval=FALSE}
...
are either `TRUE` or `FALSE`. In general, da?a analysis will involve many 
...
the next time you launch RStudio, you will se? `arbuthnot` and you will have 
...
analysis o? Arbuthnot's baptism data. Your assignment involves repeating these 
...
source("http://www.openintr?.org/stat/data/present.R")
```
...
-   How do these counts compare to Arbuthnot's? Are they on a similar scale?
  ...
  we observe that the births ?re a lot more in comparison to the Arbuthnot's Baptism 
-   Make a plot that displays the boy-to-girl ratio for every year in the data 
    ...
    greater proportion than girls hold u? in the U.S.? Include the plot in your 
    response.
...
-   In what year did we see the most total number of births in the U.S.? You can
    refer to the ?elp files or the R reference card 
    ...
    ```{r view-year_max_birds, eval=TRUE}
          present[which.max(pr?sent$boys+present$girls),]$year
    ```
...
Check it out if you would lik? to read more about an analysis of sex ratios at 
...
Feel free to bro?se around the websites for [R](http://www.r-project.org) and 
[RStudio](http://rstudio.org) if you're interested in learning more, or find 
more labs for practice at http://openintro.org.

<div id="license">
This is a product of OpenIntro that is released ?nder a 
